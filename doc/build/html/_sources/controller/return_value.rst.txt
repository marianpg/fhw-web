...
Leere Antwort
    ...
    Der Client erhält nur über den Http-Status-Code das Verarbeitungsresultat mitgeteilt. Hier findest du eine `Übersicht der Http-Status-Codes <https://de.wikipedia.org/wiki/HTTP-Statuscode>`_.


Text Antwort
    <textResult>

    Zusätzlich zum Http-Status-Code erhält der Client einen Text.


Json Antwort
    <jsonResult>

    Zusätzlich zum Http-Status-Code erhält der Client ein JSON-Objekt.
...
Promise Antwort
    ...
    Hiermit ist eine Definition einer asynchronen Controller-Funktion möglich. Der Rückgabewert der asynchronen Funktion, bzw. das Objekt, welches das Promise mittels *resolve* auflöst, hat dabei dem Rückgabewert einer Controller-Funktion zu entsprechen.
...
Eine abstrakte Syntaxbeschreibung des Rückgabeobjekts *return* ist wie folgt:
    ...
    .. code-block::
        ...
        | <promiseResult>   ::= <return> // rekursive Beschreibung, siehe oben für <return> Beschreibung
        |
        ...
        | <text>            ::= <string> // Zeichenkette, die ausgegeben wird
        | <json>            ::= <JSON> // ein Objekt, dass sich in eine json-Zeichenkette serialisieren lässt
...
**<textResult>**
    ...
    .. code-block:: js
        ...
        return {
            status: 200, // HTTP-Status-Code
            text: "Alles wird gut!" // Zeichenkette, die ausgegeben wird
        }
...
**<jsonResult>**
    ...
    .. code-block:: js
        ...
        return {
            status: 200, // HTTP-Status-Code
            json: { // ein Objekt, dass sich in eine json-Zeichenkette serialisieren lässt
                firstname: "Tina",
                lastname: "Meier"
            }
        }
...
Hier wird entweder die Controller-Funktion mit dem Schlüsselwort ``async`` als asynchrone Funktion gekennzeichnet, dann muss der Rückgabewert nicht mehr explizit als asynchron gekennzeichnet werden:

    .. code-block:: js

        const doSomething = async (data, db) => {
            /* Verarbeitungsschritte */

            return { /* Rückgabeobjekt entspricht dabei den übrigen, synchronen Rückgabewert-Definitionen */ }
        }

        module.exports = {
            "my-async-function": doSomething
        }


... oder es wird ein Promise-Objekt zurückgegeben, welches im Erfolgsfall mit dem synchronen Controller-Rückgabewert auflöst.

    .. code-block:: js

        const doSomething = (data, db) => {
            return new Promise((resolve, reject) => {
                /* Verarbeitungsschritte */

                const result = { /* Rückgabeobjekt entspricht dabei den übrigen, synchronen Rückgabewert-Definitionen */ }
                resolve(result);
            })
        }
